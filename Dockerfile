...
COPY src/ src/
RUN touch src/main.rs && cargo build --release && strip target/release/mailserver
...
RUN apk add --no-cache \
    ...
    openssl \
    && rm -rf /var/cache/apk/*
COPY --from=builder /build/target/release/mailserver /usr/local/bin/mailserver
COPY static/ /app/static/
COPY entrypoint.sh /entrypoint.sh
COPY supervisord.conf /etc/supervisord.conf
RUN chmod +x /entrypoint.sh \
    && mkdir -p /data/ssl /data/dkim /data/mail /data/db /var/spool/postfix \
    && adduser -D -s /sbin/nologin vmail \
