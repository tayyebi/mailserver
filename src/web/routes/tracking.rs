...
pub async fn list(_auth: AuthAdmin, State(state): State<AppState>) -> Html<String> {
    ...
    let pixel_base_url = load_pixel_settings(&state).await;
...
async fn load_pixel_settings(state: &AppState) -> String {
    if let Some(base) = state.blocking_db(|db| db.get_setting("pixel_base_url")).await {
        return base;
    }
    if let Ok(env_val) = std::env::var("PIXEL_BASE_URL") {
        return env_val;
    }
    // Derive a default from the server's own hostname and port
    let host = &state.hostname;
    let port = state.admin_port;
    if port == 443 {
        format!("https://{}/pixel?id=", host)
    } else if port == 80 {
        format!("http://{}/pixel?id=", host)
    } else {
        format!("https://{}:{}/pixel?id=", host, port)
    }
}
...
pub async fn update_pixel_settings(
    ...
    info!(
        "[web] POST /tracking/pixel — update pixel base url for username={}",
        auth.admin.username
    );
    let base_url = form.pixel_base_url.trim().to_string();
    if base_url.is_empty() {
        return Redirect::to("/tracking").into_response();
    }
    let base_for_db = base_url.clone();
    state
        .blocking_db(move |db| db.set_setting("pixel_base_url", &base_for_db))
        .await;
    info!(
        "[web] pixel_base_url updated to {} by user={}",
        base_url, auth.admin.username
    );
